...
class opcodetab {
    ...
    public:
        ...
        // found in the table.
        std::string get_machine_code(std::string opcode);
        ...
        // throws an opcode_error_exception if the opcode is not 
        // found in the table.        
        int get_instruction_size(std::string opcode);
                        
    private:

        const static unsigned int NUM_OPCODES = 59;
        const std::string opcodes[NUM_OPCODES] = {
        ...
        const std::string machineCode[NUM_OPCODES] = {
        ...
        const int opcodeLenth[NUM_OPCODES] = {
            3, 3, 2, 3, 2, 3, 3, 2, 3, 3,
            2, 1, 1, 1, 3, 3, 3, 3, 3, 3,
            3, 3, 3, 3, 3, 3, 3, 3, 3, 3,
            2, 1, 3, 3, 2, 3, 2, 2, 1, 3,
	    ...
	    std::map <std::string, std::string> opcodeToMachineLUT;
	    std::map <std::string, int> opcodeToSizeLUT;
};
#endif  
